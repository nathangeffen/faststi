...
static void special_case_time_step(const struct fsti_config *config,
                                   struct fsti_config_entry *entry)
{
    unsigned val;
...
static void special_cases(const struct fsti_config *config,
                          struct fsti_config_entry *entry)
{
...
void fsti_config_test(struct test_group *tg)
{
    ...
    fsti_config_add(&config, "time_step", "description", "day");
    entry = fsti_config_find(&config, "time_step");
    TESTEQ(entry != NULL, true, *tg);
    TESTEQ(entry->len, 1, *tg);
    TESTEQ(entry->variants[0].type, LONG, *tg);
